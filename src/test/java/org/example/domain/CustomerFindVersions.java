...
//  }
  ...
  public void oracle_findVersionsBetween() throws SQLException {
    ...
    //long epochStart = OffsetDateTime.now().minusMonths(10).minusDays(1).minusMinutes(250).toInstant().toEpochMilli();
    long epochStart = OffsetDateTime.now().minusDays(7).toInstant().toEpochMilli();
    Timestamp start = new Timestamp(epochStart);

    long epochEnd = OffsetDateTime.now().toInstant().toEpochMilli();
